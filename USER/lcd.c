...
u16 LCD_ReadPoint(u16 x,u16 y)
{
	...
	opt_delay(2);	  																														//延时
 	...
 	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510)									//要分2次读出
 	{
		opt_delay(2);	  
		b=LCD_RD_DATA(); 
...
{	 
	...
	{
		if(lcddev.dir==1)Xpos=lcddev.width-1-Xpos;//调转x,y坐标
...
{	   
	...
	{
 		if(lcddev.dir==1)x=lcddev.width-1-x;//调转x,y坐标
